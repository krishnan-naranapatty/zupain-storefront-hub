...
interface EditProductCouponDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  coupon: any;
}

const EditProductCouponDrawer: React.FC<EditProductCouponDrawerProps> = ({ isOpen, onClose, coupon }) => {
  ...
  const handleSave = () => {
    console.log('Saving product/category coupon:', formData);
    onClose();
  };
      ...
      <SheetContent className="w-[900px] sm:max-w-[900px] overflow-y-auto bg-gray-50" hideCloseButton={true}>
        <SheetHeader className="mb-6 pb-4 border-b border-gray-200 bg-white rounded-lg p-4 -mx-6 -mt-6 mb-6">
          <SheetTitle className="flex items-center text-xl font-semibold text-gray-900">
            ...
            {isNewCoupon ? 'Create Product/Category Coupon' : 'Edit Product/Category Coupon'}
          </SheetTitle>
        </SheetHeader>
...
export default EditProductCouponDrawer;
